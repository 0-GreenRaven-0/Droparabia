...
// .wrangler/tmp/pages-OPkk1E/functionsWorker-0.18336487779635002.mjs
...
__name2(base64urlFromString, "base64urlFromString");
...
async function getAccessToken(env) {
  ...
  cachedToken = { accessToken: data.access_token, expiresAt: Date.now() + data.expires_in * 1e3 };
...
async function onRequestGet({ env }) {
  try {
    ...
    const reviews = rows.map(([date, name, rating, text]) => ({
      date: typeof date === "string" ? date : "",
      name: typeof name === "string" ? name : "",
      rating: Number.parseInt(rating, 10),
      text: typeof text === "string" ? text : ""
    })).filter((r) => r.name && r.text && Number.isInteger(r.rating) && r.rating >= 1 && r.rating <= 5).reverse();
  ...
  try {
    ...
    const res = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        values: [[(/* @__PURE__ */ new Date()).toISOString(), name.trim(), parsedRating, text.trim(), ""]]
      })
    });
...
// .wrangler/tmp/bundle-aUxpHG/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-aUxpHG/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.18336487779635002.js.map
